...
tar xvzf %SOURCE16 && ln -sv maxLibQt-* maxLibQt
...
%build
CMAKE_OPTS="-DCMAKE_NO_SYSTEM_FROM_IMPORTED=ON -DGVARS=YES -DLUA=YES -DHELI=YES -DMULTIMODULE=YES -DPPM_LIMITS_SYMETRICAL=YES -DAUTOSWITCH=YES -DAUTOSOURCE=YES -DPPM_CENTER_ADJUSTABLE=YES -DFLIGHT_MODES=YES -DOVERRIDE_CHANNEL_FUNCTION=YES -DFRSKY_STICKS=YES -DDEBUG=YES -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS:BOOL=OFF -DINSTALL_GTEST=OFF -DINSTALL_GMOCK=OFF -DFETCHCONTENT_SOURCE_DIR_GOOGLETEST=$(pwd)/googletest -DFETCHCONTENT_SOURCE_DIR_MAXLIBQT=$(pwd)/maxLibQt"
